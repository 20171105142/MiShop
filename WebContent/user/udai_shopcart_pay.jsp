<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="cn.edu.imnu.mishop.bean.Address"%>
...
<jsp:useBean id="UsersDAO"
	class="cn.edu.imnu.mishop.dao.UsersDAO" scope="request">
</jsp:useBean>
...
<head>
	...
	<script src="<%=request.getContextPath() %>/js/swiper.min.js" charset="UTF-8"></script>
	<script src="<%=request.getContextPath() %>/js/global.js" charset="UTF-8"></script>
	<script src="<%=request.getContextPath() %>/js/jquery.DJMask.2.1.1.js" charset="UTF-8"></script>
	<title>U袋网</title>
</head>
<body>
	<!-- 顶部tab -->
				...
				<form action="" class="shopcart-form__box">
					<div class="addr-radio">
						<%
							...
							List<Address> list = UsersDAO.getUsersAddress(users.getUsers_id());
							if (!list.isEmpty()) {
								for (Address address : list) {
									if(address.getAddresses_isDefaultAddress() == 1){
						%>
										<div class="radio-line radio-box active">
											<label class="radio-label ep" title="<%=address.getAddresses_receiverAddress() %><%=address.getAddresses_receiverDetailAddress() %>（<%=address.getAddresses_receiverName() %> 收）<%=address.getAddresses_receiverPhone() %>">
												<input name="addr" checked="" value="0" autocomplete="off" type="radio"><i class="iconfont icon-radio"></i>
												<%=address.getAddresses_receiverAddress() %>
												<%=address.getAddresses_receiverDetailAddress() %>
												（<%=address.getAddresses_receiverName() %> 收）
												<%=address.getAddresses_receiverPhone() %>
											</label>
					...
					<div class="shop-order__detail">
						<table class="table">
							...
							<tbody>
								<%
								ArrayList<Cart> subList = (ArrayList)request.getSession().getAttribute("goodsList");
								System.out.println("获取提交商品信息");
									for(Cart cart : subList){
										double total = cart.getGoods_price() * cart.getGoods_amout();
								%>
								
										<tr>
										<th scope="row"><a href="item_show.html"><div class="img"><img src="<%=cart.getGoods_cover() %>" style="width:124px;height:124px;"alt="" class="cover"></div></a></th>
										<td>
					...
					<div class="shop-cart__info clearfix">
						<div class="pull-left text-left">
							...
							%>
							<div class="info-line text-nowrap">购买时间：<span class="c6"><%=now %></span></div>
						...
						<div class="pull-right text-right">
							...
							$(document).ready(function() {
								...
								$(".total").each(function () {
									...
									amount = amount + ($(".total").attr("value") * 1);
								})
								console.log(amount)
								$(".fz18 .cr").text("¥"+amount)
							})
					...
					<div class="pay-mode__box">
						...
						<div class="radio-line radio-box">
							<label class="radio-label ep">
								...
								<img src="<%=request.getContextPath() %>/images/icons/alipay.png" alt="支付宝支付">
							</label>
							<div class="pay-value">支付<b class="fz16 cr">18.00</b>元</div>
						</div>
						<div class="radio-line radio-box">
							<label class="radio-label ep">
								...
								<img src="<%=request.getContextPath() %>/images/icons/paywechat.png" alt="微信支付">
							</label>
							<div class="pay-value">支付<b class="fz16 cr">18.00</b>元</div>
						</div>
					</div>
	...
	<div class="footer">
		<div class="footer-tags">
			<div class="tags-box inner">
				<div class="tag-div">
					<img src="<%=request.getContextPath() %>/images/icons/footer_1.gif" alt="厂家直供">
				</div>
				<div class="tag-div">
					<img src="<%=request.getContextPath() %>/images/icons/footer_2.gif" alt="一件代发">
				</div>
				<div class="tag-div">
					<img src="<%=request.getContextPath() %>/images/icons/footer_3.gif" alt="美工活动支持">
				</div>
				<div class="tag-div">
					<img src="<%=request.getContextPath() %>/images/icons/footer_4.gif" alt="信誉认证">
				</div>
			</div>
		</div>
